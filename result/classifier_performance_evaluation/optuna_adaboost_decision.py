...
def process_single_fold_ada(args):
    category, train_df, ada_params, base_tree_params, train_idx, test_idx, fold_idx, model_dir = args
    ...
    ada = AdaBoostClassifier(
        estimator=DecisionTreeClassifier(**base_tree_params),
...
def objective(trial):
    ...
    base_tree_params = {
        'max_depth': trial.suggest_int('base_max_depth', 1, 5),
    ...
    for category in categories:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(skf.split(range(len(train_df)), y)):
            all_tasks.append((category, train_df, ada_params, base_tree_params,
