...
def evaluate_category_et(category, X_train, X_test, train_df, test_df):
    """
    Train and evaluate ExtraTrees model for a specific category
    ...
    et = ExtraTreesClassifier(
        ...
        min_samples_leaf=1, 
        min_weight_fraction_leaf=0.0030605268601344767, 
        max_features=None, 
        bootstrap=False, 
...
def main():
    """
    Main function to load data, prepare features, and evaluate ExtraTrees models
    ...
    for category in categories:
        train_score, test_score = evaluate_category_et(
            category, X_train_vec, X_test_vec, train_df, test_df
        )
        et_train.append(train_score)
        et_test.append(test_score)
    
    # Calculate and print average scores
    print("\n## Average scores across all categories ##")
    print(f"Average Training F1: {np.mean(et_train):.4f}")
    print(f"Average Test F1: {np.mean(et_test):.4f}")
